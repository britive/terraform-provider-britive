...
func TestBritiveResourceType(t *testing.T) {
	name := "AT - Britive Resource Manager Test Resource Type"
	description := "AT - Britive Resource Manager Test Resource Type Description"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBritiveResourceTypeExists("britive_resource_type.new_resource_type_1"),
				),
			},
		},
	})
...
func testAccCheckBritiveResourceTypeConfig(name, description string) string {
	return fmt.Sprintf(`
	resource "britive_resource_type" "new_resource_type_1" {
		name        = "%s"
		description = "%s"
		parameters {
