...
func NewNotFoundErrorf(format string, a ...interface{}) error {
	return fmt.Errorf("%w %s", britive_client.ErrNotFound, fmt.Sprintf(format, a...))
}
...
// NewNotSupportedError - godoc
func NewNotSupportedError(resource string) error {
	return fmt.Errorf("%s is not supported", resource)
}
