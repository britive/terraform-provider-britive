...
* **New Resource:** `britive_application_scan_schedule` : Create, update, and manage scheduled scans (`Hourly`/`Daily`/`Weekly`/`Monthly`) for an application, with optional environment/environment-group scoping (`scope` accepts a name or ID, mirroring `britive_profile`'s `associations` block). Schema mirrors `britive_resource_manager_resource_type_schedule_scan` (`day_of_week`/`day_of_month`/`start_time`), plus an application-only `hour_interval` for the `Hourly` frequency type. `org_scan` is unmanaged when omitted from config, like `scan_enabled`/`rotation_enabled` above - checked against the raw config value on every apply, not the computed plan value, so a value merely carried forward from prior state (which always ends up concrete, `true` or `false`, once set) isn't mistaken for an explicit `org_scan` on a later `Update` and wrongly rejected for an application type that doesn't support it. Exported as `null` (not `false`) when the API omits `orgScan` from its response entirely, as it does for application types that don't support it at all. Scheduled scanning support, `scope`, and `org_scan` are validated against the application's actual `application_type` (e.g. `Kubernetes` does not support scheduled scanning at all; `scope`/`org_scan` are only supported for some types) - see the resource's docs for the full support matrix. Adds an `application_type` computed attribute, resolved once and cached since it never changes for a given application. Two `scope` blocks referencing the same environment/environment group via different forms (one by name, one by ID) are tracked independently on refresh, avoiding a "Duplicate Set Element" planning error that would otherwise occur from the second `terraform plan`/`apply` onward.
* **Resource:** `britive_application` : Added a `scan_enabled` argument to enable or disable all scheduled scans for the application; added a `task_service_id` computed attribute used internally by `britive_application_scan_schedule`. `scan_enabled` is unmanaged when omitted from config, so upgrading does not affect existing applications. Setting `scan_enabled` for an `application_type` that does not support scheduled scanning at all (currently only `Kubernetes`) fails `terraform apply`. If resolving `task_service_id` at creation fails because the backend hasn't finished provisioning the scan task service yet (e.g. `E1004: Task service does not exist ...`), the application and everything else configured in that apply are still saved to state (with `task_service_id`/`scan_enabled` left at their not-yet-resolved defaults) instead of being rolled back - a subsequent `terraform apply` resolves both automatically via the resource's normal refresh, with no `terraform destroy`/recreate or duplicate-object conflict.
