...
* `script_content` - (Optional) Inline rotation script content. Required when `template_type = "InlineFile"`; must be unset otherwise. The provider re-reads the live content on every `terraform plan`/`refresh`, so an out-of-band edit made directly on the backend shows up as drift and gets reverted to match this value on the next `apply`.
