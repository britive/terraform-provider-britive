...
func (c *Client) ExecuteProfilePermissionRequest(ctx context.Context, profileID string, ppr ProfilePermissionRequest) error {
	url := fmt.Sprintf("%s/paps/%s/permissions", c.APIBaseURL, profileID)
	_, err := c.Post(ctx, url, ppr, ProfileLockName)
	if err != nil {
		return err
	}
