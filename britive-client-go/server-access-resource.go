...
func (c *Client) UpdateServerAccessResource(serverAccessResource ServerAccessResource) (*ServerAccessResource, error) {
	...
	req, err := http.NewRequest("PUT", fmt.Sprintf("%s/resource-manager/resources/%s", c.APIBaseURL, serverAccessResource.ResourceID), strings.NewReader(string(serverAccessResourceBody)))
	if err != nil {
		return nil, err
	}
